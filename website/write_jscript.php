<?
	...
	*/
	function write_jscript($date, $this_page)
	{
		...
		print("		{\n");
		print("			open( url,\"\",\"toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes,width=710,height=655\");\n");
		print("		}\n");
		print("		function OpenSoho( url )\n");
		print("		{\n");
		print("			open( url,\"\",\"toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=no,width=710,height=720\");\n");
		print("		}\n");
		print("		function OpenAce( url )\n");
		print("		{\n");
		print("			open( url,\"\",\"toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=no,width=710,height=720\");\n");
		print("		}\n");
		print("		function OpenEve( url )\n");
		print("		{\n");
		print("			open( url,\"\",\"toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=no,width=710,height=720\");\n");
		print("		}\n");
		...
		print("		{\n");
		print("           open(\"${arm_data_path}/data/${dirdate}/meta/noaa_events_raw_${date}.txt\",\"new_window\",\"toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=y,scrollbars=yes\");\n");
		...
		print("		}\n");	
		print("		function OpenHek(url)\n");
		print("		{\n");
		print("			open(url,\"new_window\",\"toolbar=yes,location=yes,directories=yes,status=no,menubar=yes,resizable=yes,scrollbars=yes,width=800,height=890,left=3,top=20\");\n");
		print("		}\n");	
		print("		function OpenAurora( url )\n");
		print("		{\n");
		print("			open( url,\"\",\"toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes,width=710,height=700\");\n");
		print("		}\n");
		...
		if (($this_page == "full_disk.php")||(isset($_GET['region'])))
		{
			...
			print("		{\n");
			print("			open( url,\"new_window\",\"toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=y,scrollbars=yes,width=710,height=755\");\n");
			print("		}	\n");
		}        	
		
		if($this_page == "slideshow.php")
		{
			$dir = "${arm_data_path}data/$dirdate/pngs";
			...
			foreach($fd_types as $type)
			{
				...
				if(file_exists("${arm_data_path}data/$dirdate/pngs/$instrument/$file"))
				{
					print("		Pic[$i] = '${arm_data_path}data/$dirdate/pngs/$instrument/$file'\n");
					$i++;	
				}
			}

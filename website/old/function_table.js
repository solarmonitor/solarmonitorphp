...
function rowOver(i,j, nColor) {
  if (!nColor) nColor = "#ECECD9";
  var nameObj = (document.getElementById) ? document.getElementById('name' + i +  j) : eval("document.all['name" + i +  j + "']");
  if (nameObj != null) nameObj.style.background=nColor;
}
function rowOut(i,j, nColor) {
  var trObj = (document.getElementById) ? document.getElementById('ihtr' + i + j) : eval("document.all['ihtr" + i +  j +  "']");
  var nameObj = (document.getElementById) ? document.getElementById('name' + i +  j) : eval("document.all['name" + i +  j + "']");
  if (trObj == null || trObj.style.display=="none") nameObj.style.background=nColor;
}
function servOC(i,j, href, nColor) {
  var trObj = (document.getElementById) ? document.getElementById('ihtr' + i + j) : eval("document.all['ihtr" + i + j + "']");
  var nameObj = (document.getElementById) ? document.getElementById('name' + i +  j) : eval("document.all['name" + i + j + "']");
  var ifObj = (document.getElementById) ? document.getElementById('ihif' + i + j) : eval("document.all['ihif" + i + j + "']");
  if (trObj != null) {
    if (trObj.style.display=="none") {
      ...
      smoothHeight('ihif' + i + j, 0, 210, 42, 'o');
    }
    else {
      nameObj.style.background=nColor;
      smoothHeight('ihif' + i + j, 210, 0, 42, 'ihtr' + i + j);
    }
  }
}

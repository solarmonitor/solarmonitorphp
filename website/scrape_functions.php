<?
	...
	function scrape_ticker_most_likely_to_flare($date)
	{
		...
		else
		{
			...
			$item["link"] = "./index.php?date=" . $date . "&region=" . $most_likely["number"];
		}
	...
	function scrape_ticker_most_active_region($date)
	{
		...
		else
		{
			...
			$item["link"] = "./index.php?date=" . $date . "&region=" . $most_active["number"];			
		}
	...
	function scrape_rss_activity_level($date)
	{
		...
		if ($regions == false)
		{
			...
			$item["c_count"]="?";
			$item["flare_index"] = "?";
			$item["link"] = "./goes_pop.php?date=$date&amp;type=xray";
			return($item);	
		}

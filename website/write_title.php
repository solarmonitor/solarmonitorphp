	...
	*/
	...
	function write_title($date, $title, $this_page, $indexnum, $type=NULL, $width=780, $region=NULL)
	{
		...
		if ($pop_up !== false)
			...
			elseif ($indexnum == "3")
				$human_readable_date_line = "<a class=mail4 href=\"./index3.php?date=$date\">$human_readable_date</a>";
			elseif ($indexnum == "4")
				$human_readable_date_line = "<a class=mail4 href=\"./index4.php?date=$date\">$human_readable_date</a>";
			else
		...
		if ($this_page == 'forecast.php'){ //&& $this_page == 'forecast.php'){strtotime($date) <= strtotime("20150831")
